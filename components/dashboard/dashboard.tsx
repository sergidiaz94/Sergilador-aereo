...
import { OlgaModal } from '../olga/olga-modal';
...
export default function Dashboard() {
  ...
  const [olgaModalOpen, setOlgaModalOpen] = useState(false);
  const userCoords = useGeolocation();

  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col selection:bg-pink-500 selection:text-white">
      <DashboardHeader onOlgaClick={() => setOlgaModalOpen(true)} />
      ...
      <OlgaModal open={olgaModalOpen} onClose={() => setOlgaModalOpen(false)} />
    </div>
  );
}
