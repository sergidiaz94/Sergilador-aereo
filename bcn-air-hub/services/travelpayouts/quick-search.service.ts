...
// Cerca ràpida origen -> Barcelona (funció secreta "Ets l'Olga?"). Es
// comproven diverses destinacions de l'àrea de Barcelona alhora (BCN, però
// també Girona i Reus, on Ryanair sol operar vols "Barcelona" barats), i
// es combinen tots els resultats. Retorna les millors opcions concretes
// dels pròxims `daysAhead` dies, i per separat el preu més barat trobat en
// cache sense restricció de data.
export async function findCheapestDirectFlights(
  ...
  if (!hasTravelpayouts()) {
    const demo = sortByDate(demoOlgaOffers(origin, destinations[0]));
  ...
  const preciseResults = await Promise.all(
    destinations.map((destination) => searchByExactDates(origin, destination, dates)),
  );
  const broadResults = await Promise.all(
    destinations.map((destination) => searchByCache(origin, destination)),
  );

  const preciseOffers = preciseResults.flat();
  const broadOffers = broadResults.flat();

  let nextDays: FlightOffer[] = [];

  if (preciseOffers.length > 0) {
    nextDays = sortByDate([...preciseOffers].sort((a, b) => a.price - b.price).slice(0, 3));
  } else if (broadOffers.length > 0) {
    nextDays = sortByDate([...broadOffers].sort((a, b) => a.price - b.price).slice(0, 3));
  } else {
    const skyscannerResults = await Promise.all(
      destinations.map((destination) => searchRoundtripSkyscanner(origin, destination, dates[0])),
    );
    const skyscannerOffers = skyscannerResults.flat();
    nextDays = skyscannerOffers.length > 0 ? sortByDate(skyscannerOffers.sort((a, b) => a.price - b.price).slice(0, 3)) : [];
  }
