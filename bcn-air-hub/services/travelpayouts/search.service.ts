...
// Cerca manual (com un cercador a l'estil Skyscanner): destí i dates
// escollits per l'usuari. Fa servir /v1/prices/direct, que retorna fins a
// 3 opcions (directe, 1 escala, 2 escales) per a les dates exactes donades.
export async function searchFlightPrices(
  ...
  if (!hasTravelpayouts()) {
    return demoFlightOffers({ origin: BCN_AIRPORT.code, destination, departureDate: departDate, returnDate });
  }
  ...
  try {
    ...
    return offers.length > 0
      ? offers
      : demoFlightOffers({ origin: BCN_AIRPORT.code, destination, departureDate: departDate, returnDate });
  } catch (error) {
