...
export async function getCheapestWeekendDeals(origin: string = DEFAULT_ORIGIN): Promise<FlightDeal[]> {
  if (!hasTravelpayouts()) {
  ...
  try {
    ...
    for (const result of results) {
      ...
      deals.push({
        ...
        currency: 'EUR',
        deepLink: buildAviasalesLink(origin, destination, cheapest.departure_at ?? friday, cheapest.return_at ?? sunday),
        source: 'live',
      });
    }
