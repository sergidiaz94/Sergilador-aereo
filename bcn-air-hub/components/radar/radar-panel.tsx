...
export function RadarPanel({ userCoords }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 bg-slate-950 rounded-2xl border border-slate-800 relative overflow-hidden h-[450px] flex items-center justify-center">
        <div className="absolute inset-0 opacity-15 bg-[radial-gradient(#3b82f6_1px,transparent_1px)] [background-size:16px_16px]" />

        <div className="absolute top-4 left-4 bg-slate-900/90 border border-slate-700 p-2.5 rounded-xl text-xs flex items-center gap-2 backdrop-blur z-20">
        ...
        {isLoading ? (
          ...
          <RadarMap flights={flights} onSelect={setSelectedFlight} />
        )}
