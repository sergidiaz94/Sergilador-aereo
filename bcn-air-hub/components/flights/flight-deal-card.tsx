...
type Props = { deal: FlightDeal; featured?: boolean };

export function FlightDealCard({ deal, featured }: Props) {
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <span className="text-[11px] text-blue-300 font-bold tracking-wider block mb-1">
              MILLOR OFERTA DES DE BCN
            </span>
            <span className="text-2xl font-black text-white block">Barcelona (BCN) -&gt; {deal.destination}</span>
            <span className="text-xs text-slate-400">
              Anada: {deal.departureDate}
              {deal.returnDate ? ` | Tornada: ${deal.returnDate}` : ''}
            </span>
          </div>
  ...
  return (
    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 flex justify-between items-center flex-wrap gap-4 hover:border-blue-500/40 transition-all">
      <div>
        <span className="text-xs text-blue-400 font-bold block">{deal.destination}</span>
        <span className="text-xs text-slate-400">
          Anada: {deal.departureDate}
          {deal.returnDate ? ` | Tornada: ${deal.returnDate}` : ''}
        </span>
